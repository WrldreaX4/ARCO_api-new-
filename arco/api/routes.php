...
if ($_SERVER['REQUEST_METHOD'] == 'OPTIONS') {
    ...
    header("Access-Control-Allow-Methods: OPTIONS, GET, POST, PUT, DELETE, FETCH");
    ...
    case 'GET':
        switch ($request[0]) {
            case 'get_signup':
                ...
                case 'projectreport': // Fix the typo here
                    $result = [];
                    if (count($request) > 1) {
                        // Call the correct method: get_project_report() instead of projectreport()
                        $result = $get->projectreport($request[1]);
                    } else {
                        $result = $get->projectreport();
                    }
    ...
    case 'POST':
        ...
        switch ($request[0]) {
            ...
            case 'project_status_report';
                $result = $post->projectStatusReport($data);
                echo json_encode($result);
                break;
